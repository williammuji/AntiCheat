#include "CheatCheatConfigManager.h"
...
const std::vector<std::wstring>& CheatConfigManager::GetWhitelistedWindowKeywords() const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void CheatConfigManager::SetDefaultValues() {
    ...
    m_config->add_whitelisted_window_keywords(L"visual studio");
    m_config->add_whitelisted_window_keywords(L"obs");
    m_config->add_whitelisted_window_keywords(L"discord");
...
std::string CheatConfigManager::GetServerPublicKey() const {
    ...
    const unsigned char g_encryptedPublicKey[] = {
        0x75, 0x6d, 0x66, 0x77, 0x48, 0x73, 0x45, 0x0c, 0x57, 0x65, 0x73, 0x7e, 0x0c, 0x41, 0x63, 0x61, 
        0x6c, 0x49, 0x63, 0x79, 0x0c, 0x5d, 0x69, 0x6e, 0x6e, 0x49, 0x67, 0x73, 0x0c, 0x46, 0x6f, 0x72, 
        0x0c, 0x4b, 0x6e, 0x7e, 0x66, 0x43, 0x68, 0x65, 0x61, 0x7e, 0x0c, 0x53, 0x7d, 0x73, 0x7e, 0x65, 
        0x6d, 0x0c, 0x3d, 0x0c, 0x1f, 0x1c
    };
...
std::string CheatConfigManager::EncryptData(const std::string& plain_text) const {
    ...
    // 使用 CRYPTPROTECT_LOCAL_MACHINE 使加密与机器绑定
    if (CryptProtectData(&in, L"AntiCheatConfig", NULL, NULL, NULL, CRYPTPROTECT_LOCAL_MACHINE, &out)) {
...
bool CheatConfigManager::VerifySignature(const anti_cheat::ClientConfig& config) const {
    if (!config.has_config_signature()) return false;
